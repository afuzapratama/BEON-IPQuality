...
CREDS
    ...
    # Create GeoIP.conf if MaxMind is configured
    if [[ -n "$MAXMIND_ACCOUNT_ID" && "$MAXMIND_ACCOUNT_ID" != "skip" ]]; then
        print_progress "Creating GeoIP.conf..."
        cat > $INSTALL_DIR/configs/GeoIP.conf << GEOIPCONF
# MaxMind GeoIP Configuration
# Generated by BEON-IPQuality installer
...
GEOIPCONF
        ...
        print_success "GeoIP.conf created"
    fi
...
CRONGEOIP
    ...
    if [[ "$ENTRY_COUNT" -gt 0 ]]; then
        ...
        if sudo -u $USER $INSTALL_DIR/bin/compiler \
            ...
            print_info "You can retry later: sudo -u beon $INSTALL_DIR/bin/compiler -oneshot"
        fi
    else
    ...
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    local SERVER_IP=$(curl -4 -s --max-time 5 icanhazip.com 2>/dev/null || curl -4 -s --max-time 5 ipecho.net/plain 2>/dev/null || hostname -I | awk '{print $1}')
